...
cmd({
    pattern: "trt2",
    alias: ["translate2"],
    desc: "☹️ Translate text between languages",
    react: "⚡",
    category: "other",
    filename: __filename
},
async (conn, mek, m, { from, q, reply }) => {
    try {
        ...
        const url = `https://delirius-apiofc.vercel.app/tools/translate?text=${encodeURIComponent(textToTranslate)}&to=${targetLang}`;

        const { data } = await axios.get(url);

        if (!data.status) return reply("⚠️ Translation failed. Please try again later.");
